    ...
    <div className="flex h-full">
      <aside className="flex w-[220px] shrink-0 flex-col border-r border-line bg-sidebar">
        ...
        <nav className="flex-1 space-y-0.5 overflow-y-auto px-2.5">
          {apps.map((app) => {
            const Icon = app.icon;
            const isActive = !settingsOpen && app.id === activeApp.id;
            return (
              <button
                key={app.id}
                type="button"
        ...
        <div className="border-t border-line p-2.5">
          <button
            ...
            className={[
              "flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-ui transition-colors duration-100",
              settingsOpen
                ? "bg-accent-soft font-medium text-accent"
                : "text-ink-muted hover:bg-raised hover:text-ink",
            ].join(" ")}
      ...
      <main className="flex min-w-0 flex-1 flex-col bg-canvas">
        <header
          ...
          {...TITLEBAR_DRAG}
        >
          {settingsOpen ? "设置" : activeApp.name}
        </header>
